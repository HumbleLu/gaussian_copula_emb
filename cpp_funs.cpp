...
arma::mat c_eff_cpp(arma::mat alpha,
  ...
  for(int n = (n_len - 1); n >=0; n--) {
    ...
    for(int l = (w_len - 1); l >=0; l--){
      t = context_index_n[l] - 1;
      val = context_val_n[l];
      
      for(int p = (p_len - 1); p >=0; p--){
        // Rcpp::Rcout << alpha(v, p) << std::endl;
        out_mtx(n,p) += alpha(t, p) * val * norm_const;
      }
    }
  }
  return(out_mtx);
...
arma::mat grad_phi_cpp(arma::vec x_val,
  ...
  arma::vec phi_t;
  arma::mat R_n;
  double val_n, d_z_n, mu, d_mu, d_phi;
  for(int n = (n_len-1); n >= 0; n--){
...
arma::mat grad_alpha_cpp(arma::vec x_val,
  ...
  arma::mat R_n, phi_t;
  double mu, d_mu, d_z, d_alpha, val_center, val_context, norm_const;
...
// [[Rcpp::export]]
double log_dplacketluce_lkhd_cpp(arma::vec x, arma::vec score) {
  ...
  arma::vec denom(len);
  double sum = arma::accu(exp_score_ord);
  for(int i = 0; i < len; i++) {
    for(int j = i; j < len; j++){
      denom[i] += exp_score_ord[j];
    }
  }
  
  // Rcpp::Rcout << "exp_score_ord: "<< exp_score_ord << std::endl;
  // Rcpp::Rcout << "denom: "<< denom << std::endl;
  
  double out = 0;
  for(int i = 0; i < len; i++) {
    out += log(exp_score_ord[i] / denom[i]);
  }
...
// [[Rcpp::export]]
arma::vec grad_log_placketluce_cpp(arma::vec x, arma::vec score) {
  int len = score.n_elem;
  arma::uvec order = sort_index(x, "descend");
  arma::vec exp_score = exp(score);
  arma::vec exp_score_ord = exp_score(order);
  ...
  for(int i = 0; i < len; i++) {
    out_vec[i] = exp_score_ord[i] * (1/exp_score_ord[i] - accu(1/denom.subvec(0,i)));
  }
